...
user function Emprestimos()
	...
	BEGIN SEQUENCE
		...
		AADD(aCores,{"Z6_STATUS == 'D'",'BR_AZUL_MDI'})			//Devolvido
...
Static Function fLinhaOk(nOpc)
	...
	SZ5->(dbSetOrder(1))		//Z5_CODIGO+Z5_ACERVO
...
Static Function fDevolver(nOpc)
	...
	SZ7->(dbSetOrder(2))		//Z7_PROTOCO+Z7_EXEMPLA
		...
		If SZ7->(MsSeek(SZ6->Z6_PROTOCO))
			While !SZ7->(Eof()) .And. SZ7->Z7_PROTOCO == SZ6->Z6_PROTOCO
				If Empty(SZ7->Z7_DEVREAL)
					AADD(aVetor,{.F.,SZ7->Z7_EXEMPLA,AllTrim(GetAdvFVal("SZ4","Z4_TITULO",GetAdvFVal("SZ5","Z5_ACERVO",SZ7->Z7_EXEMPLA,1,""),1,"")),SZ7->Z7_DEVPREV})
				EndIf
				SZ7->(dbSkip())
			End
		EndIf
